...
export const createPost = async (req: Request, res: Response) => {
  ...
  let existCode = true
  while (existCode) {
    code = generateRandomString(2).toUpperCase() + generateRandomNumber(4);
  ...
  for (const item of req.body.items) {
    ...
    if(productDetail) {
      ...
      if(item.attributeValue) {
        ...
        const variantMatched = productDetail.variants.find(variantItem => {
          return (
            variantItem.attributeValue.every((attr: any) => {
              const selected = item.attributeValue.find((v: any) => v.attriId === attr.attriId);
        ...
        for (const v of item.attributeValue) {
