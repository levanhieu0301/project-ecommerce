import { Router } from "express";
const router = Router();
import * as roleController from "../../controllers/admin/role.controller"
import * as roleValidate from "../../validates/admin/role.validate"
import { checkPermissions } from "../../middlewares/admin/account.middleware";
import multer from "multer";
const upload = multer();
router.get('/create', roleController.role)
router.post('/create', upload.none(),checkPermissions("role-create"),roleValidate.createPost, roleController.roleCreatePost)
router.get('/list', roleController.roleList)
router.get('/edit/:id', roleController.roleEdit)
router.patch('/edit/:id', upload.none(),checkPermissions("role-edit"), roleValidate.createPost, roleController.roleEditPatch)
router.patch('/delete/:id', roleController.roleDeletePatch)
router.get('/trash', roleController.roleTrash)
router.patch('/undo/:id', roleController.roleUndo)
router.delete('/destroy/:id', roleController.roleDestroy)

export default router;